...
void initializeDatabase() {
  ...
  if (sql.connect()) {
    sql.execute("CREATE TABLE IF NOT EXISTS Account (user_id int AUTO_INCREMENT, username varchar(150), coins int, PRIMARY KEY(user_id));");
    ...
    sql.execute("CREATE TABLE IF NOT EXISTS Item (itemid int AUTO_INCREMENT, name varchar(200), value int, level int, PRIMARY KEY (itemid));");
    sql.execute("CREATE TABLE IF NOT EXISTS Inventory (user_id int, itemid int, PRIMARY KEY (user_id, itemid), FOREIGN KEY (user_id) REFERENCES Account(user_id), "
